...
public class Project1{
	/* This defines every pentomino, in all possible formatations and rotations
	*/	
	public static void main(String[] args){
		...
		/*Added Hashmap so we could actually index every pentomino
		*/
		...
		/* Scanner code that asks for user's input to create a grid
		*/
		...
		/* some basecase conditions for the grid, like min. and max. area size
		*If area isn't divisble by 5 cant completely fill it
		*/
		if(area%5 != 0){
		...
		/*Short piece that asks for the pentominoes that have to be used
		*
		*/
	...
	/*Method that actually places the pentominoes
	*/
	public static void placePentomino(char[][] grid, char[][][][] pentSet, char[] pentsUsed, int progress){
		...
		*including the method to check if pieces go out of bound(out of the grid)
			...
			for (int y = 0; y < grid[0].length; y++){
				for (int variant = 0; variant < pentSet[pentIndex].length; variant++){
					...
					if (checkOutOfBounds(grid, pent, pentSet, variant, x, y) == true){
						...
						}// int solCount = 0 solCount++ to check for solutions?
					}
				}
			}
		}
	...
	*/
	...
	/*Methock checks if pieces go out of bound
	*/
	...
	/*Checks if the pentominoes actually overlap each other
	*/
